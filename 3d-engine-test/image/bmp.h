...
struct bmp : object {
  ...
  void construct( size_t width = defwidth, size_t height = defheight ) {
    info = make_object_heap<bmpinfo>();

    info->width = width;
    info->height = height;
    size_t img_size = width * height * sizeof( uint32_t );

    BITMAPFILEHEADER bfh_ = { 'M' << 8 | 'B', DWORD( img_data_offset + img_size ), 0, 0, DWORD( img_data_offset ) };
    info->bfh = bfh_;

    BITMAPINFO& bmi = info->bmi;
    ...
    info->data = memdata<uint32_t>{ img_size };
  }

  void destruct() {
    if( info != nullptr ) 
      free_object_heap<bmpinfo>( info );
  }
  ...
  ok setdibits( const ui::window& wnd ) {
    ...
    if( !StretchBlt( hdc, 0, 0, wnd.width(), wnd.height(), memdc, 0, 0, width(), height(), SRCCOPY ) ) {
      ok_ = make_notok( windows::getLastErrorString() );
    }
