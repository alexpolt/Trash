...
namespace alexpolt { namespace util {
  ...
  */
  ...
  template<class A, class B> struct is_base_of_history {
  ...
  template<class A, class B> struct is_base_of_version2 { // is_base_of_version2<A, A>::value == 1 => true
    static char isbaseof( A* );
    static int isbaseof( ... );
    static const int value = sizeof(  isbaseof( (A*)0 )  ) == 1;
  };
