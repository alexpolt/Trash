...
#define test_return( result, ret ) if( ! result ) { result.log(); return ret; }
#define log_return( result, ret ) result.log(); return ret;
#define test_terminate( result ) if( ! result ) result.terminate();
  ...
  template<class T> struct result {
    ...
    void log() { error::logError(); }

    void terminate() { _exit( 1 ); }
